...
First, all changes to the [ocm-api-model](https://github.com/openshift-online/ocm-api-model) have been defined and reviewed. Than, the client types for the model need to be generated via `make update` target in the `ocm-api-model` project.
...
Once all changes to the OCM API Model have been committed to the main branch and a new tag is pused a new release will be created automatically.Than, you will need to update these changes in the SDK. (See "Updating the OCM SDK" section)
