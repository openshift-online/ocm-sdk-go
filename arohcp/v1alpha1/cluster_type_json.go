...
func WriteCluster(object *Cluster, stream *jsoniter.Stream) {
	...
	present_ = object.bitmap_&8388608 != 0 && object.disabledCapabilities != nil
	if present_ {
		if count > 0 {
			stream.WriteMore()
		}
		stream.WriteObjectField("disabled_capabilities")
		WriteOptionalClusterCapabilityList(object.disabledCapabilities, stream)
		count++
	}
...
func ReadCluster(iterator *jsoniter.Iterator) *Cluster {
	...
	for {
		...
		case "disabled_capabilities":
			value := ReadOptionalClusterCapabilityList(iterator)
			object.disabledCapabilities = value
