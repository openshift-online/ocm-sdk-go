...
func readAttachmentsListRequest(request *AttachmentsListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeAttachmentsListResponse(response *AttachmentsListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(AttachmentListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeAttachmentList(response.items.items, stream)
			count++
		}
	}
