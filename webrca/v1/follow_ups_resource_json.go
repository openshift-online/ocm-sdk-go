...
func readFollowUpsListRequest(request *FollowUpsListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
	...
	request.followUpStatus, err = helpers.ParseString(query, "follow_up_status")
	if err != nil {
		return err
	}
...
func writeFollowUpsListResponse(response *FollowUpsListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(FollowUpListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeFollowUpList(response.items.items, stream)
			count++
		}
	}
