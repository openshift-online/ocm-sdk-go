...
func (c *ClusterLogsClient) Add() *ClusterLogsAddRequest {
	request := new(ClusterLogsAddRequest)
...
func (c *ClusterLogsClient) List() *ClusterLogsListRequest {
	request := new(ClusterLogsListRequest)
	request.transport = c.transport
	request.path = c.path
	request.metric = c.metric
	return request
...
func (r *ClusterLogsAddRequest) SendContext(ctx context.Context) (result *ClusterLogsAddResponse, err error) {
	...
	result = new(ClusterLogsAddResponse)
...
// 'add' method.
func (r *ClusterLogsAddResponse) unmarshal(reader io.Reader) error {
	var err error
...
func (r *ClusterLogsListRequest) SendContext(ctx context.Context) (result *ClusterLogsListResponse, err error) {
	...
	defer response.Body.Close()
	result = new(ClusterLogsListResponse)
	result.status = response.StatusCode
	result.header = response.Header
	if result.status >= 400 {
		result.err, err = errors.UnmarshalError(response.Body)
...
// unmarshal is the method used internally to unmarshal responses to the
// 'list' method.
func (r *ClusterLogsListResponse) unmarshal(reader io.Reader) error {
	var err error
	decoder := json.NewDecoder(reader)
	data := new(clusterLogsListResponseData)
	err = decoder.Decode(data)
	if err != nil {
		return err
	}
...
// clusterLogsListResponseData is the structure used internally to unmarshal
// the response of the 'list' method.
type clusterLogsListResponseData struct {
