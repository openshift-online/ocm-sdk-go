...
// LogsAdapter is an HTTP handler that knows how to translate HTTP requests
// into calls to the methods of an object that implements the LogsServer
// interface.
type LogsAdapter struct {
	server LogsServer
}

// NewLogsAdapter creates a new adapter that will translate HTTP requests
// into calls to the given server.
func NewLogsAdapter(server LogsServer) *LogsAdapter {
	return &LogsAdapter{
		server: server,
	}
}
...
func (a *LogsAdapter) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	dispatchLogsRequest(w, r, a.server, helpers.Segments(r.URL.Path))
}

// dispatchLogsRequest navigates the servers tree rooted at the given server
// till it finds one that matches the given set of path segments, and then invokes
// the corresponding server.
func dispatchLogsRequest(w http.ResponseWriter, r *http.Request, server LogsServer, segments []string) {
	if len(segments) == 0 {
		switch r.Method {
		case http.MethodGet:
		...
		default:
			...
			dispatchLogRequest(w, r, target, segments[1:])
		}
	}
}
