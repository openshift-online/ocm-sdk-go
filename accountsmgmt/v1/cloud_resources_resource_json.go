...
func writeCloudResourcesListResponse(response *CloudResourcesListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(CloudResourceListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeCloudResourceList(response.items.items, stream)
			count++
		}
	}
