...
// This example shows how to retrieve the collection of incidents
...
func main() {
	...
	for {
		response, err := collection.List().
			//Note("O adsit ob turpibus spe eliqua, cui oceani, hi cor extra intime.").
			Size(size).
			Page(page).
			SendContext(ctx)
		...
		response.Items().Each(func(event *web.Event) bool {
			fmt.Printf("%s - %s - %s\n", event.Incident().ID(), event.Note(), event.EventType())
			return true
		})
