...
func main() {
	...
	resource := collection.V1().AccessRequests()

	object, err := v1.NewAccessRequestPostRequest().
		ClusterId("xxx").
		Justification("xxx").
		InternalSupportCaseId("xxx").
		Build()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't build access request: %v\n", err)
		os.Exit(1)
	}

	response, err := resource.Post().Body(object).
		SendContext(ctx)
	if err != nil {
	...
	var b bytes.Buffer
	v1.MarshalAccessRequest(response.Body(), &b)
