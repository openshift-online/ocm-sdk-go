...
// This example shows how to list account group assignments.
...
func main() {
	...
	// Send the request to list account group assignments:
	response, err := collection.List().
		Size(100).
		SendContext(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't list account group assignments: %v\n", err)
		os.Exit(1)
	}

	// Print the result:
	response.Items().Each(func(assignment *amv1.AccountGroupAssignment) bool {
