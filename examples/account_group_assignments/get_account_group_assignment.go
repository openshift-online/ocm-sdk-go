...
// This example shows how to retrieve an account group assignment.
...
func main() {
	...
	// Get the client for the resource that manages the account group assignment that we are looking for. Note
	// that this will not send any request to the server yet, so it will succeed even if that
	// assignment doesn't exist.
	resource := collection.AccountGroupAssignment("32qCo6grq14r6e5CJ0obahpSRRw")

	// Send the request to retrieve the account group assignment:
	response, err := resource.Get().SendContext(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't retrieve account group assignment: %v\n", err)
		os.Exit(1)
	}
