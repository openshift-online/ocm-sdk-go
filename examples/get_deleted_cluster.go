...
func main() {
	...
	deletedCluster, err := deletedClustersClient.DeletedCluster("2nqieidaj9ej38fu52ou08luomq2ai89").Get().SendContext(ctx)
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Can't retrieve deleted cluster: %v\n", err)
		os.Exit(1)
	}

	// Perform next actions with the retrieved deleted cluster
	fmt.Println(deletedCluster)
}
