...
// This example shows how to retrieve an account group.
...
func main() {
	...
	// Get the client for the resource that manages the account group that we are looking for. Note
	...
	// Send the request to retrieve the account group:
	response, err := resource.Get().SendContext(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't retrieve account group: %v\n", err)
		os.Exit(1)
	}
