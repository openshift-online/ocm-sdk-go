...
// This example shows how to create an account group.
...
func main() {
	...
	// Prepare the description of the account group to create:
	accountGroup, err := amv1.NewAccountGroup().
		Name("mygroup").
		Description("Account group created by OCM SDK").
		Build()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't create account group description: %v\n", err)
		os.Exit(1)
	}

	// Send a request to create the account group:
	response, err := collection.Add().
		Body(accountGroup).
		SendContext(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't create account group: %v\n", err)
		os.Exit(1)
	}

	// Print the result:
	accountGroup = response.Body()
	fmt.Printf("%s - %s\n", accountGroup.ID(), accountGroup.Name())
}
