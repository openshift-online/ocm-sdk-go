...
// This example shows how to delete an account group.
...
func main() {
	...
	// Get the client for the resource that manages the account group that we want to delete. Note
	...
	// Send the request to delete the account group:
	_, err = resource.Delete().SendContext(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't delete account group: %v\n", err)
		os.Exit(1)
	}
}
