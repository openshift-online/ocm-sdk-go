...
// This example shows how to list account groups.
...
func main() {
	...
	// Send the request to list account groups:
	response, err := collection.List().
		Size(100).
		SendContext(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't list account groups: %v\n", err)
		os.Exit(1)
	}

	// Print the result:
	response.Items().Each(func(accountGroup *amv1.AccountGroup) bool {
